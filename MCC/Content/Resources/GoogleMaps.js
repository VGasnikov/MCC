(function(){
  ...
  function loadfunc() {
    if( ! GBrowserIsCompatible() ) {
      if( document.getElementsByTagName ) {
        // Find all divs marked as cpMapId
        var divs = document.getElementsByTagName( 'div' );
        for( var i = 0; i < divs.length; ++i ) {
          var div = divs[ i ];
          if( div.className.indexOf( 'cpMapId' ) > -1 ) {
            div.innerHTML =  [
    ...
    var markerOpener = {
      ...
      chainOpen: function( i ) {
        ...
        if( i < this.markers.length ) {
          ...
          //alert('cpMapId loadtime: ' + ( new Date() - startdate ) );
        }
      }
    };
    ...
    // Find all anchor tags linking to cpMapId markers
    ...
    for( var i = 0; i < divs.length; ++i ) {
      ...
      if( 'cpMapId' in classes ) {
        new EZMap( div, classes );
      }
    }

    lastFunc = function() {
      // Examine current page location for a reference to a cpMapId marker
  ...
  document.write( [
    ...
    'div.cpMapId .firefoxref {',
    '  text-align: center;',
    '  margin: 1em;',
    '}',

    'div.cpMapId .loadprogress {',
    '  position: relative;',
    '  z-index: 100000000;',
    '  text-align: center;',
    '}',

    'div.cpMapId .loadprogress .box {',
    ...
    'div.cpMapId .loadprogress .trough {',
    '  margin: 8px;',
    '  margin-top: -5px;',
    '  background: #ccc;',
    '  font-size: 1px;',
